...
import { API_URL, regex } from '../constants/constants';
...
export default function Film() {
  ...
  async function fetchFilm() {
    ...
    const response: any = await axios.get<IShows>(`${API_URL}/${params.id}`);
...
const getActors = () => {
  ...
  async function fetchActors() {
    const response: any = await axios.get(`${API_URL}/${params.id}/cast`);
...
const getSeasons= () => {
  ...
  async function fetchSeasons() {
    const response: any = await axios.get(`${API_URL}/${params.id}/seasons`);
...
const getEpisodes= () => {
  ...
  async function fetchEpisode() {
    const response: any = await axios.get(`${API_URL}/${params.id}/episodes`);
    ...
    <>
        <div className="filmBg bg-gradient-to-r from-custom-dark-blue to-gray-800 h-auto min-h-screen pt-32">
            ...
            <div className="w-full lg:w-2/5">
                <img src={poster || 'Not Found'} alt="" className="rounded-none lg:rounded-lg shadow-2xl hidden lg:block w-96"/>
